...
public class Game24 {
	...
	private boolean checkcal(double a, double b, int op) {
		if (op == '/' && b < 1e-6 && b > -1e-6)
			return false;
		if (op == '-' && b < 1e-6 && b > -1e-6)
			return false;
	...
	// 求值
	private double calculate(double a, double b, int op) {
		switch (op) {
		case '+':
			return a + b;
		case '-':
			...
			if (data[i].operType == 0) {
				...
				if (suffix.peek().operType == 0) {
					...
					if (!suffix.empty() && suffix.peek().operType == 0) {
						...
						suffix.pop();
						if (!checkcal(b, a, data[i].operType))
							return -1;
						double c = calculate(b, a, data[i].operType);
	...
	public Set<String> start(int data[], boolean bAll) {
		...
		int[][] sequence = new int[][] { new int[] { 1, 1, 1, 1, 2, 2, 2 },
